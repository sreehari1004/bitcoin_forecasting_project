...
from models.rt_model import train_rt
...
model = train_rt(X_train, y_train, max_depth=10)

# 4. Evaluate the model
rmse, predictions = evaluate_model(model, X_test, y_test, model_type='sklearn')
print(f"RT RMSE: {rmse:.4f}")

# 5. Plot predictions
plot_predictions(y_test, predictions, title="Regression Tree: Forecast vs Actual")

# Save the rmse for comparison
with open('Results/RT_rmse.txt', 'w') as file:
    file.write(str(rmse))
...
print(f"RT R² score: {r2:.4f}")

# 8. Calculate MAPE (Mean Absolute Percentage Error)
mape = np.mean(np.abs((y_test - predictions) / y_test)*100)
accuracy = 100 - mape
print(f"RT Accuracy: {accuracy:.4f}%")

# 9. Save the R² score and Accuracy into the result file
with open('Results/RT.txt', 'w') as file:
    file.write(f"RT: {rmse:.4f}\n")
